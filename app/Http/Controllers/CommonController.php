...
class CommonController extends Controller
{
    ...
    public function loadTrendTracker()
    {
        $year = date('Y');
        $month = date('m');
        $trackingdata = TrackingIncrement::whereMonth('date', $month)->whereYear('date', $year)->orderBy('date', 'ASC')->get();
        foreach($trackingdata as $row){
            $carbonDate = Carbon::parse($row->date);
            $dateViews[] = $carbonDate->day;
            $dataViews[] = $row->tracking_increment;
        }
        ...
        $response = array(
            ...
            'dateViews' => $dateViews,
            'dataViews' => $dataViews,
            'monthYear' => strtoupper(Shortcut::medium_bulan($month)).' '.$year,
        );
        return response()->json($response);
    }
